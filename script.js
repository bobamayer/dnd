(() => {
  ...
  function renderSetup(mode, players) {
    ...
    if (mode === 'edit' && players && players.length) {
      ...
      if (!cancelEditBtn) {
        ...
        cancelEditBtn.className = 'btn btn-tertiary btn-block';
  ...
  function updateStatsHeader() {
    ...
    const escaped = state.players.filter((p) => p.treasure >= p.goal).length;
    const treasureSum = state.players.reduce((sum, p) => sum + p.treasure, 0);

    document.querySelector('[data-stat-adventurers]').textContent = String(total);
    document.querySelector('[data-stat-escaped]').textContent = `${escaped}/${total}`;
    document.querySelector('[data-stat-treasure]').textContent = formatNumber(treasureSum);
  }
