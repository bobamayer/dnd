...
- Big **+ / −** buttons plus a one-tap button for every treasure card denomination (250 gp through 10,000 gp) so you rarely need to type a value.[^2]
- A progress bar fills up as each character gets closer to their goal.
...
- Uses the "Even Keel" design system (Lora + Public Sans, muted teal/plum palette, Compact density) for visual consistency with the rest of the portfolio.
- Works offline after the first load, except the Google Fonts (Lora, Public Sans) which need an internet connection the first time; after that they're cached by the browser.
...
[^2]: Treasure card values by dungeon level, per the component list: [Dungeon Component List and Card Values](https://www.scribd.com/document/431110043/Dungeon-Component-List-and-Card-Values-pdf).
